...
export class Rule {
  ...
  static movesOnSameLine(currRow, currCol, boardStates) {
    const moves = [];
    for (let i = currRow + 1; i <= this.maxRow; i++) {
      const k = [i, currCol].toString();
      if (k in boardStates) {
        if (!boardStates[k][1]) {
    ...
    for (let j = currRow - 1; j >= this.minRow; j--) {
      const k = [j, currCol].toString();
      if (k in boardStates) {
        if (!boardStates[k][1]) {
    ...
    for (let i = currCol + 1; i <= this.maxCol; i++) {
      const k = [currRow, i].toString();
      if (k in boardStates) {
        if (!boardStates[k][1]) {
    ...
    for (let j = currCol - 1; j >= this.minCol; j--) {
      const k = [currRow, j].toString();
      if (k in boardStates) {
        if (!boardStates[k][1]) {
  ...
  static possibleMovesForPao(currRow, currCol, boardStates, team) {
    ...
    for (let i = currRow + 1; i <= this.maxRow; i++) {
      const k = [i, currCol].toString();
      if (k in boardStates) {
        const next = this.findFirstOpponentOnCol(currCol, i + 1, boardStates, team, inc);
        if (next) {
          moves.push(next);
        }
        break;
    ...
    for (let j = currRow - 1; j >= this.minRow; j--) {
      const k = [j, currCol].toString();
      if (k in boardStates) {
        const next = this.findFirstOpponentOnCol(currCol, j - 1, boardStates, team, dec);
        if (next) {
          moves.push(next);
        }
        break;
    ...
    for (let i = currCol + 1; i <= this.maxCol; i++) {
      const k = [currRow, i].toString();
      if (k in boardStates) {
        const next = this.findFirstOpponentOnRow(currRow, i + 1, boardStates, team, inc);
        if (next) {
          moves.push(next);
        }
        break;
    ...
    for (let j = currCol - 1; j >= this.minCol; j--) {
      const k = [currRow, j].toString();
      if (k in boardStates) {
        const next = this.findFirstOpponentOnRow(currRow, j - 1, boardStates, team, dec);
        if (next) {
          moves.push(next);
        }
        break;
  ...
  static getGameEndStateByState = function (myPieces: Piece[], oppoPieces: Piece[], boardState, team) {
    const myKing = myPieces.filter(x => x.name === 'k')[0];
    const oppoKing = oppoPieces.filter(x => x.name === 'k')[0];
    if (!myKing) {
      return -1;
    }
    if (!oppoKing) {
    ...
    const myKingCol = myKing.position[1];
    // not on the same col
    ...
    if (team === 1) {
      minRow = myKing.position[0] + 1;
      maxRow = oppoKing.position[0] - 1;
    } else {
      minRow = oppoKing.position[0] + 1;
      maxRow = myKing.position[0] - 1;
    }
...
export class Agent {
  ...
  // agent take action
  nextMove() {
    const computeResult = this.comptuteNextMove();
    const piece = computeResult[0];
    const toPos = computeResult[1];
...
export class State {
  ...
  getEndState() {
    const playing = this.playingTeam === 1 ? this.redAgent : this.blackAgent;
    const endState = Rule.getGameEndState(playing);
    return endState;
  }
  // return a copy of state
  copy(setOppoo = true) {
    const newState = new State(this.redAgent.copy(), this.blackAgent.copy(), this.playingTeam, setOppoo);
    return newState;
  }
